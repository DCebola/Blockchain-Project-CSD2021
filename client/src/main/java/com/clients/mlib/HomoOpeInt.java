...
public class HomoOpeInt {
	...
	public double hgd(long kk, long nn1, long nn2,  Random rand) {
        ...
        if (nn1 < 0 || nn2 < 0 || kk < 0 || kk > nn1 + nn2) { throw new java.lang.ArithmeticException("Math Error: DOMAIN");
        //  	return Double.NaN;
        }
        ...
        if (minjx == maxjx) {
            ...
            if (setup1 || setup2) {
                if (k < n2) {
                    ...
                    w = java.lang.Math.exp(con + afc(n2) + afc(n1 + n2 - k) - afc(n2 - k) - afc(n1 + n2));
                }
                else {
                    /*!* 		w = exp(con + afc(n1) + afc(k) *!*/
                    w = java.lang.Math.exp(con + afc(n1) + afc(k) - afc(k - n2) - afc(n1 + n2));
                }
            }
            L10: while (true) {
                p = w;
            ...
            if (setup1 || setup2) {
                ...
                kl = java.lang.Math.exp(conta);
                /*!* 	    kr = exp(a - afc((int) (xr - 1)) *!*/
                conta = a - afc((long) (xr - 1)) - afc((long) (n1 - xr + 1)) - afc((long) (k - xr + 1)) - afc((long) (n2 - k + xr - 1));
                //System.out.println("Conta: "+conta);
                if(conta > 5) conta = 5;//limita��o da exponencial
                kr = java.lang.Math.exp(conta);
                /*!* 	    lamdl = -log(xl * (n2 - k + xl) / (n1 - xl + 1) *!*/
                lamdl = -java.lang.Math.log(xl * (n2 - k + xl) / (n1 - xl + 1) / (k - xl + 1));
                /*!* 	    lamdr = -log((n1 - xr + 1) * (k - xr + 1) *!*/
                lamdr = -java.lang.Math.log((n1 - xr + 1) * (k - xr + 1) / xr / (n2 - k + xr));
                p1 = d + d;
                p2 = p1 + kl / lamdl;
                p3 = p2 + kr / lamdr;
            ...
            L30: while (true) {
                ...
                if (u < p1) {
                    ...
                    /* left tail */
                    /*!* 	    ix = xl + log(v) / lamdl; *!*/
                    ix = (long) (xl + java.lang.Math.log(v) / lamdl);
                    ...
                    /* right tail */
                    /*!* 	    ix = xr - log(v) / lamdr; *!*/
                    ix = (long) (xr - java.lang.Math.log(v) / lamdr);
	...
	double afc(double i) {
		...
		if (i < 0) {
			...
			value = (di + 0.5) * java.lang.Math.log(di) - di + 0.399089934;
